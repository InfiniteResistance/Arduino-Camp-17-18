...
Adafruit_DCMotor *dcMotor2 = AFMS.getMotor(2);
...
void configurePins()
{
  ...
  arduinoPins[5].type=pwm;                    //motor 1
  arduinoPins[6].type=pwm;                    //motor 2
...
void updateActuator(byte pinNumber)
{
  if (arduinoPins[pinNumber].type==digital) 
  {
    if (pinNumber == 10)
    {
      if (arduinoPins[pinNumber].state == HIGH)
      dcMotor1->run(FORWARD);
      else
  ...
  else if (arduinoPins[pinNumber].type==pwm) 
  {
    if (pinNumber == 5)
    {      
      dcMotor1->setSpeed(arduinoPins[pinNumber].state/2);
    }
    else if (pinNumber == 6)
    {
      dcMotor2->setSpeed(arduinoPins[pinNumber].state/2);
    }
    else
    {
